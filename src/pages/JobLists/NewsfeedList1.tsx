...
import { NEWSFEED_DATA } from "../TabPages/Newsfeed";
import {
  logoTwitter,
  logoInstagram,
  send,
...
import useFirebaseUpload from "../../hooks/NewsfeedFileUpload";
import "./font.css";
const NewsfeedList1: React.FC = () => {
  const ListNewsfeed1JobDetailsId = useParams<{ newsfeed1Id: string }>().newsfeed1Id;
  const [showToast1, setShowToast1] = useState(false);
  const ListNewsfeed1JobDetails = NEWSFEED_DATA.find(
    (job) => job.id === ListNewsfeed1JobDetailsId
  );
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          {!isPlatform("android") && (
            <IonButtons slot="end">
              <IonButton href={"mailto:Goofle@gmail.com"}>
                <IonIcon slot="icon-only" ios={send} md={send}></IonIcon>
              </IonButton>
            </IonButtons>
          )}
          <IonTitle>{ListNewsfeed1JobDetails?.title}</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent>
        <IonGrid className="auto">
          <IonRow>
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {ListNewsfeed1JobDetails?.about.map((about) => (
            ...
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {ListNewsfeed1JobDetails?.overview.map((overview) => (
            ...
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {ListNewsfeed1JobDetails?.requirements.map((requirements) => (
            ...
            <IonCol size-xs="12" size-md="4.5" offset-md="1.5">
              {ListNewsfeed1JobDetails?.skills.map((skills) => (
            ...
            <IonCol size-xs="12" size-md="4.5">
              {ListNewsfeed1JobDetails?.contact.map((contact) => (
