...
import { BUSINESS_DATA } from "../SearchPages/SearchBusiness";
import {
  logoTwitter,
  logoInstagram,
  send,
...
import useFirebaseUpload from "../../hooks/BusinessFileUpload";
import "./font.css";
const ListBusiness: React.FC = () => {
  const listBusinessJobDetailsId = useParams<{ businessId: string }>()
    .businessId;
  const [showToast1, setShowToast1] = useState(false);
  const listBusinessJobDetails = BUSINESS_DATA.find(
    (job) => job.id === listBusinessJobDetailsId
  );
  ...
  return (
    <IonPage>
      <IonHeader>
        <IonToolbar>
          {/* The IonButtons component is simply a containor element primarly used for placing an IonButton in to help distinguish ppositioning left to right. */}
          ...
          <IonTitle>{listBusinessJobDetails?.title}</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent>
        <IonGrid className="auto">
          <IonRow>
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {listBusinessJobDetails?.about?.map((about) => (
            ...
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {listBusinessJobDetails?.overview.map((overview) => (
            ...
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {listBusinessJobDetails?.requirements.map((requirements) => (
            ...
            <IonCol size-xs="12" size-md="4.5" offset-md="1.5">
              {listBusinessJobDetails?.skills.map((skills) => (
            ...
            <IonCol size-xs="12" size-md="4.5">
              {listBusinessJobDetails?.contact.map((contact) => (
