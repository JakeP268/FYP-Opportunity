...
import { SCIENCE_DATA } from "../SearchPages/SearchScience";
import {
  logoTwitter,
  logoInstagram,
  send,
...
import useFirebaseUpload from "../../hooks/ScienceFileUpload";
import "./font.css";
const ListScience: React.FC = () => {
  const listScienceJobDetailsId = useParams<{ scienceId: string }>().scienceId;
  const [showToast1, setShowToast1] = useState(false);
  const listScienceJobDetails = SCIENCE_DATA.find((job) => job.id === listScienceJobDetailsId);
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          <IonTitle>{listScienceJobDetails?.title}</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent>
        <IonGrid className="auto">
          <IonRow>
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {listScienceJobDetails?.about.map((about) => (
            ...
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {listScienceJobDetails?.overview.map((overview) => (
            ...
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {listScienceJobDetails?.requirements.map((requirements) => (
            ...
            <IonCol size-xs="12" size-md="4.5" offset-md="1.5">
              {listScienceJobDetails?.skills.map((skills) => (
            ...
            <IonCol size-xs="12" size-md="4.5">
              {listScienceJobDetails?.contact.map((contact) => (
