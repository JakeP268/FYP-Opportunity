...
import { MARKETING_DATA } from "../SearchPages/SearchMarketing";
import {
  logoTwitter,
  logoInstagram,
  send,
...
import useFirebaseUpload from "../../hooks/MarketingFileUpload";
import "./font.css";
const ListMarketing: React.FC = () => {
  const ListMarketingJobDetailsId = useParams<{ marketingId: string }>().marketingId;
  const [showToast1, setShowToast1] = useState(false);
  const ListMarketingJobDetails = MARKETING_DATA.find(
    (job) => job.id === ListMarketingJobDetailsId
  );
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          <IonTitle>{ListMarketingJobDetails?.title}</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent>
        <IonGrid className="auto">
          <IonRow>
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {ListMarketingJobDetails?.about.map((about) => (
            ...
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {ListMarketingJobDetails?.overview.map((overview) => (
            ...
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {ListMarketingJobDetails?.requirements.map((requirements) => (
            ...
            <IonCol size-xs="12" size-md="4.5" offset-md="1.5">
              {ListMarketingJobDetails?.skills.map((skills) => (
            ...
            <IonCol size-xs="12" size-md="4.5">
              {ListMarketingJobDetails?.contact.map((contact) => (
