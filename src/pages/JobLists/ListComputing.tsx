...
import { COMPUTING_DATA } from "../SearchPages/SearchComputing";
import {
  logoTwitter,
  logoInstagram,
  send,
...
import useFirebaseUpload from "../../hooks/ComputingFileUpload";
import "./font.css";
const ListComputing: React.FC = () => {
  const listComputingJobDetailsId = useParams<{ computingId: string }>()
    .computingId;
  const [showToast1, setShowToast1] = useState(false);
  const listComputingJobDetails = COMPUTING_DATA.find(
    (job) => job.id === listComputingJobDetailsId
  );
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          <IonTitle>{listComputingJobDetails?.title}</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent>
        <IonGrid className="auto">
          <IonRow>
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {listComputingJobDetails?.about.map((about) => (
            ...
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {listComputingJobDetails?.overview.map((overview) => (
            ...
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {listComputingJobDetails?.requirements.map((requirements) => (
            ...
            <IonCol size-xs="12" size-md="4.5" offset-md="1.5">
              {listComputingJobDetails?.skills.map((skills) => (
            ...
            <IonCol size-xs="12" size-md="4.5">
              {listComputingJobDetails?.contact.map((contact) => (
