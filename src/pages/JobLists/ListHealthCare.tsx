...
import { HEALTHCARE_DATA } from "../SearchPages/SearchHealthCare";
import {
  logoTwitter,
  logoInstagram,
  send,
...
import useFirebaseUpload from "../../hooks/HealthCareFileUpload";
import "./font.css";
const ListHealthCare: React.FC = () => {
  const ListHealthCareJobDetailsId = useParams<{ healthcareId: string }>().healthcareId;
  const [showToast1, setShowToast1] = useState(false);
  const ListHealthCareJobDetails = HEALTHCARE_DATA.find(
    (job) => job.id === ListHealthCareJobDetailsId
  );
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          <IonTitle>{ListHealthCareJobDetails?.title}</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent>
        <IonGrid className="auto">
          <IonRow>
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {ListHealthCareJobDetails?.about.map((about) => (
            ...
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {ListHealthCareJobDetails?.overview.map((overview) => (
            ...
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {ListHealthCareJobDetails?.requirements.map((requirements) => (
            ...
            <IonCol size-xs="12" size-md="4.5" offset-md="1.5">
              {ListHealthCareJobDetails?.skills.map((skills) => (
            ...
            <IonCol size-xs="12" size-md="4.5">
              {ListHealthCareJobDetails?.contact.map((contact) => (
