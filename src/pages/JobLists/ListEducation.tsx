...
import { EDUCATION_DATA } from "../SearchPages/SearchEducation";
import {
  logoTwitter,
  logoInstagram,
  send,
...
import useFirebaseUpload from "../../hooks/EducationFileUpload";
import "./font.css";
const ListEducation: React.FC = () => {
  const listEducationJobDetailsId = useParams<{ educationId: string }>()
    .educationId;
  const [showToast1, setShowToast1] = useState(false);
  const listEducationJobDetails = EDUCATION_DATA.find(
    (job) => job.id === listEducationJobDetailsId
  );
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          <IonTitle>{listEducationJobDetails?.title}</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent>
        <IonGrid className="auto">
          <IonRow>
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {listEducationJobDetails?.about.map((about) => (
            ...
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {listEducationJobDetails?.overview.map((overview) => (
            ...
            <IonCol size-xs="12" size-md="9" offset-md="1.5">
              {listEducationJobDetails?.requirements.map((requirements) => (
            ...
            <IonCol size-xs="12" size-md="4.5" offset-md="1.5">
              {listEducationJobDetails?.skills.map((skills) => (
            ...
            <IonCol size-xs="12" size-md="4.5">
              {listEducationJobDetails?.contact.map((contact) => (
